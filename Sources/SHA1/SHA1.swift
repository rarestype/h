...
@frozen public struct SHA1: Equatable, Hashable, Sendable {
    @usableFromInline typealias Storage = (UInt32, UInt32, UInt32, UInt32, UInt32)
...
extension SHA1: CustomStringConvertible {
    @inlinable public var description: String { self.buffer.description }
}
extension SHA1: LosslessStringConvertible {
    ...
    @inlinable public init?(_ description: borrowing Substring) {
        if  let buffer: InlineBuffer<Storage> = .init(description) {
            self.init(buffer: buffer)
        } else {
            return nil
...
extension SHA1: ExpressibleByIntegerLiteral {
    @inlinable public init(integerLiteral: StaticBigInt) {
        self.init(buffer: .init(integerLiteral: integerLiteral))
    }
}
extension SHA1 {
    @available(*, unavailable, message: "unimplemented")
    @inlinable public init(hashing message: __shared some Collection<UInt8>) {
        fatalError()
    }
}
