...
@frozen public struct SHA256: Sendable {
    public typealias Words = (UInt32, UInt32, UInt32, UInt32, UInt32, UInt32, UInt32, UInt32)
    ...
    @inlinable public init<Message>(hashing message: Message) where Message: Collection<UInt8> {
        self.init()
    ...
    @inlinable public mutating func update<Chunk>(
        with chunk: Chunk
    ) where Chunk: Collection<UInt8> {
    ...
    @inlinable public static func rotate(_ value: UInt32, right shift: Int) -> UInt32 {
...
extension SHA256: MessageAuthenticationHash {
    public static let stride: Int = 64
    public static let count: Int = 32
...
extension SHA256: ExpressibleByStringLiteral {
    @inlinable public init?(parsing ascii: some Sequence<UInt8>) {
        guard let words: Words = Base16.decode(ascii, loading: Words.self) else {
            return nil
        }
