...
@frozen public struct CRC32: Hashable, Sendable {
    ...
    public static let table: [UInt32] = (0 ..< 256).map {
        (i: UInt32) in
    ...
    @inlinable public consuming func updated(
        with message: borrowing some Sequence<UInt8>
    ) -> Self {
    ...
    @inlinable public consuming func _updated(with message: borrowing [UInt8]) -> Self {
        var checksum: Self = self
    ...
    /// This manually specialized implementation is much faster in debug mode than the
    /// generic implementation, but exactly the same in release mode.
    @inlinable public mutating func _update(with message: borrowing [UInt8]) {
        #if DEBUG
