@frozen public struct MD5: Equatable, Hashable, Sendable {
    @usableFromInline typealias Storage = (UInt32, UInt32, UInt32, UInt32)

    @usableFromInline var buffer: InlineBuffer<Storage>

    @inlinable init(buffer: InlineBuffer<Storage>) {
        self.buffer = buffer
    }
}
extension MD5 {
...
extension MD5 {
    @inlinable public init(words: Words) {
...
extension MD5: CustomStringConvertible {
    public var description: String { self.buffer.description }
}
extension MD5: LosslessStringConvertible {
    public init?(_ description: borrowing String) {
...
extension MD5: ExpressibleByIntegerLiteral {
    @inlinable public init(integerLiteral: borrowing StaticBigInt) {
        self.init(buffer: .init(integerLiteral: integerLiteral))
    }
}
extension MD5 {
    @inlinable public init<Message>(
        hashing message: __shared Message
    ) where Message: Collection<UInt8> {
        var words: Words = .init()

        var start: Message.Index = message.startIndex
        while   let end: Message.Index = message.index(
                start,
                offsetBy: 64,
