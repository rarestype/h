...
@frozen public struct MessageAuthenticationKey<Hash> where Hash: MessageAuthenticationHash {
    public let inner: [UInt8]
    public let outer: [UInt8]

    /// Creates a message authentication key from the given base key.
    @inlinable public init(_ key: some Collection<UInt8>) {
...
extension MessageAuthenticationKey {
    ...
    /// using this key.
    @inlinable public func authenticate(_ message: some Sequence<UInt8>) -> Hash {
